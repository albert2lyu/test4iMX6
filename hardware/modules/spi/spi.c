...
static unsigned int speed = MAX_SPEED;
...
int spi_transfer(int fd, char *tbuf, char *rbuf, int bytes)
{
    ...
    LOGI("Send    data : 0x%x  ,string: %s\n", tbuf[0], (char *)tbuf);
    LOGI("receive  len : %d\n", ret);
    LOGI("receive data : 0x%x  ,string: %s\n", rbuf[0], (char *)rbuf);
...
int spi_open(int no)
{
    ...
    case 1:
    {
        ...
        if (fd < 0)
        {
            ...
            LOGE("Failed to open device file: %s -- %s.", DEV_SPI1,strerror(errno));
            return -1;
        }
        break;
    }
...
SPI_HARDWARE_MODULE_ID, name : "SPI Stub"
