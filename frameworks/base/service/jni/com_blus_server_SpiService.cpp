 ...
 * Signature: (I[B[BII)I
 */
static jint spi_transfer(JNIEnv* env, jclass thiz, jint _fd, jbyteArray _tbuf, jbyteArray _rbuf, jint _len, jint _times)
{
    ...
    else
    {
        ...
        LOGI("TBuf = %s\n",pTbuff);
#else
        env->GetByteArrayRegion(_tbuf, 0, len, (jbyte*)pTbuff);
#endif
        //===========================
        ...
        //===========================
        len = (len>BUF_MAX_SIZE)?BUF_MAX_SIZE:len;
        if(times ==0)
        {
            ...
            LOGI("RBuf = %s\n",pRbuff);
#else
            //jbyteArray array = env->NewByteArray(BUF_MAX_SIZE);
            env->SetByteArrayRegion(_rbuf, 0, BUF_MAX_SIZE,(jbyte*)pRbuff);
#endif
        }
        else
        {
            for(i=0; i<len; i++)
            {
                pTbuff_[0] = *((char*)(pTbuff+i));
                data = sSpiDevice->transfer( fd, pTbuff_, pRbuff, 1);
            ...
            env->SetByteArrayRegion(_rbuf, 0, BUF_MAX_SIZE,(jbyte*)pRbuff);
            LOGI("SpiService JNI: spi_transfer's times: %d uS",times);
        }
        free(pTbuff);
        free(pRbuff);
    }
...
static const JNINativeMethod gMethods[] =
{
    ...
    { "_transfer", "(I[B[BII)I", (void*) spi_transfer },
